...
library(DVstats)
# USGSwsBase is required by DVstats, so readNWIS,
# renCol, and screenData functions are available.
RRVM <- renCol(readNWIS("05484500", end.date="2012-09-30"))
# The screenData function is useful to review for 
# complete record, default is by calendar year.
with(RRVM, screenData(datetime, Flow))
...
RRVM7ClimY <- with(RRVM, 
  dvStat(Flow, datetime,
    by=climateYear(datetime),
    pre=movingAve,
    STAID="05484500",
    span=7, pos="trailing"))
